...
class TorrentUploadHooks{
	...
	public static function onUploadComplete(&$image){
		...
		if(MimeMagic::singleton()->isMatchingExtension('torrent', $image->getLocalFile()->mime)) {
			...
			$creationdate = $torrentInfo[2];//not used
			$name = $torrentInfo[3];
			$torrentsize = $torrentInfo[4];
			$filecount = $torrentInfo[5];
			$annlist = $torrentInfo[6];
			$comment = $torrentInfo[7];//not used
			$filelist = $torrentInfo[8];//not used, TODO could be stored for display in torrent page
			...
			$upload_torrent = array(
				...
				'info_hash' => $infohash,
				'name' => $name,
				'filename' => $fname,//what for?
				//'save_as' => ''.$fname.'',
				//'category' => ''.$catid.'',
				'size' => $torrentsize,
				//'type' => ''.$type.'',
				'numfiles' => $filecount,
				'owner' => $wgUser->getId(),//ssuming torrent is current user's
				//'nfo' => ''.$nfo.'',
				'announce' => $announce,
				//'torrentlang' => ''.$langid.''//mmm
			);
			...
			//get inserted id
			$result = $dbw->select("traffic_torrents","id","info_hash='".$infohash."'");//TODO assuming infohash is unique
